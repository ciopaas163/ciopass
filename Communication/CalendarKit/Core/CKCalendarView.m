...
@property (nonatomic, strong) CKCalendarView *database;
...
//  The index of the highlighted cell  选中的cell
...
+(CKCalendarView*)sharDatabase{   //单例
    static CKCalendarView* datadb = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (datadb ==nil) {
            datadb = [[CKCalendarView alloc]init];
        }
    });
    return datadb;
...
-(void)commonInitializer {
    self.frame = CGRectMake(0,40,KSCREENWIDTH , KSCREENHEIGHT-40);
    ...
    //  Events for selected date 选择的日期的事件
    ...
    //  First Weekday 第一个工作日
    _firstWeekDay = [_calendar firstWeekday];
    
    heeght =24;
    
//        NSString *sht = [self titleForHeader:[self headerView]];
//        NSLog(@"sht = %@",sht);
...
//    }
    ...
    /**
     *  TODO: Possibly add a delegate method here, per issue #20 可能添加委托方法.
     */
    
    /**
     *  Reload the calendar view 重装日历视图.
     */
...
#pragma mark - View Hierarchy 视图的层次结构
...
//    CGSize cellSize = [self _cellSize];
    ...
    if (displayMode == CKCalendarViewModeWeek) {
        ...
        rect.size.height += heeght;
        rect.origin.y -= heeght;
    }
...
- (CGRect)_rectForCellsForDisplayMode:(CKCalendarDisplayMode)displayMode
{
    ...
    else if(displayMode == CKCalendarViewModeWeek)
    {
        ...
        return CGRectMake(0, heeght, (CGFloat)daysPerWeek*cellSize.width, cellSize.height);
    }
    else if(displayMode == CKCalendarViewModeMonth)
    {
        ...
        return CGRectMake(0, heeght, width, height);
    }
    return CGRectZero;
}
...
{
    /*  Enforce view dimensions appropriate for given mode  执行视图尺寸适合给定的模式 */
    ...
    CGRect headerFrame = CGRectMake(0, 0, width, heeght);
    ...
    /* Set up the table 安装表*/
    
    CGRect tableFrame = [[self superview] bounds];
    tableFrame.origin.y = CGRectGetMaxY(self.frame);
    
    /**
     *  Correct for iPhone 6 and iPhone 6 Plus shadow bug    iPhone 6和iPhone 6的正确加阴影错误.
     */
...
- (void)_layoutCellsAnimated:(BOOL)animated
{
    ...
    /* Calculate the pre-animation offset 计算预动画偏移*/
    ...
    //  Count the rows and columns that we'll need  计数的行和列
    ...
    //  Cache the cell values for easier readability below 可读性缓存单元格的值
    CGFloat width = [self _cellSize].width;
    CGFloat height = [self _cellSize].height;
    
    //  Cache the start date & header offset  开始日期和标题偏移缓存
    ...
    for (NSUInteger row = 0; row < rowCount; row++) {
        for (NSUInteger column = 0; column < columnCount; column++) {
            ...
            NSUInteger months = [[self calendar] monthsInDate:workingDate];
            NSUInteger years = [[self calendar] yearsInDate:workingDate];
            [cell setNumberYear:@(years)];
            [cell setNumberMonth:@(months)];
            [cell setNumber:@(day)];
            
            
            /* STEP 5: Show event dots 显示点*/
...
#pragma mark - Cell Animation cell动画
...
#pragma mark - Cell Recycling cell循环
...
- (NSString *)titleForHeader:(CKCalendarHeaderView *)header
{
    ...
    //Otherwise, return today's date as a string 返回今天日期
...
#pragma mark - Rows and Columns 列和行
...
#pragma mark - UITableViewDataSource 日历日程
#warning 日期事件表
...
{
    self.titleHeader = [[self date]monthAndYearOnCalendar:[self calendar]];
    
    CKCalendarDisplayMode mode = [self displayMode];
    if(mode == CKCalendarViewModeMonth)
    {
   
        NSString *rl =[NSString stringWithFormat:@"%@日程",[[self date] monthNameOnCalendar:[self calendar]]];
        return rl;
    }
    ...
    {
        [self usexingqi];
        NSString *rl =[NSString stringWithFormat:@"%@日程 %@",[[self date] monthAndDayAndYearOnCalendar:[self calendar]],weekDayStr];
        return rl;
    }
    //Otherwise, return today's date as a string 返回今天日期
    NSString *rl =[NSString stringWithFormat:@"%@当天日程",[[self date] dayOfMonthOnCalendar:[self calendar]]];
    return rl;
...
#pragma mark - First Weekday 第一个工作日
...
#pragma mark - Date Calculations 日期计算
...
{
    // for the day mode, just return today 这一天的方式，只是今天还
    if (displayMode == CKCalendarViewModeDay) {
        return [self date];
    }
    else if(displayMode == CKCalendarViewModeWeek)
...
{
    //  If we're showing one day, well, we only one 如果我们展示一天，嗯，我们只有一个
    if (displayMode == CKCalendarViewModeDay) {
        return 1;
    }
...
#pragma mark - Minimum and Maximum Dates 最小和最大的日期
...
#pragma mark - Dates & Indices 日期及指标
...
#pragma mark - Touch Handling 触摸操作
...
#pragma mark If a touch was cancelled, reset the index 如果触摸被取消，重新索引
...
#pragma mark 判断选中日期是星期几
-(void)usexingqi
{
    
    NSCalendar *gregorian = [[NSCalendar alloc]
                             initWithCalendarIdentifier:NSChineseCalendar];
    NSDateComponents *weekdayComponents = [gregorian components:NSWeekdayCalendarUnit fromDate:[self date]];
