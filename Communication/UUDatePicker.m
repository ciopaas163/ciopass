...
- (CGFloat)pickerView:(UIPickerView *)pickerView widthForComponent:(NSInteger)component
{
    switch (self.datePickerStyle) {
        case UUDateStyle_YearMonthDayHourMinute:{
            return self.frame.size.width/5 - 15;
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view
{
    ...
    switch (self.datePickerStyle) {
        case UUDateStyle_YearMonthDayHourMinute:{
            if (component==0) {
                title = [yearArray[row] stringByAppendingString:@"年"];
                textColor = [self returnYearColorRow:row];
            }
            if (component==1) {
                title = [monthArray[row] stringByAppendingString:@"月"];
                textColor = [self returnMonthColorRow:row];
            }
            if (component==2) {
                title = [dayArray[row] stringByAppendingString:@"日"];
                textColor = [self returnDayColorRow:row];
            }
            if (component==3) {
                title = [hourArray[row] stringByAppendingString:@"时"];
                textColor = [self returnHourColorRow:row];
            }
            if (component==4) {
                title = [minuteArray[row] stringByAppendingString:@"分"];
