...
+(NSArray *)GetDayArrayByYear:(int) year andMonth:(int) month{
    ...
    //NSLog(@"%d",(((2016/4==0&&2016/100!=0)||(2016/400==0))&&(month==2)));
    int lastMonthDays=0;
    if (month==1)
    {
        lastMonthDays=[self GetNumberOfDayByYera:year-1 andByMonth:12];
    }else
    {
        lastMonthDays=[self GetNumberOfDayByYera:year andByMonth:month-1];
    }
    int currWeekday=[self GetTheWeekOfDayByYera:year andByMonth:month];
    lastMonthDays=lastMonthDays-currWeekday+1;
    int nextDays=1;
    for (int i = 0; i< 42; i++) {
        if (i < currWeekday) {
            [dayArray addObject:[NSString stringWithFormat:@"%d",lastMonthDays]];
            lastMonthDays++;
            //[dayArray addObject:@" "];
        }else if ((i>currWeekday-1)&&(i<currWeekday+[self GetNumberOfDayByYera:year andByMonth:month])){
            NSString * days;
            ...
            [dayArray addObject:[NSString stringWithFormat:@"%d",nextDays]];
            nextDays++;
        }
    }
    return dayArray;
